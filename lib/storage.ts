...
/** Migrate legacy notes (folderId → folderIds) and ensure new fields */
...
export async function deleteFolder(id: string): Promise<boolean> {
  ...
  // Remove this folderId from all notes
...
export async function updateNote(
  id: string,
  updates: Partial<Pick<Note, "content" | "folderIds" | "amount" | "expenseCategory" | "recordedAt" | "todos">>
): Promise<Note | null> {
  const notes = await getNotes();
...
> {
  const notes = await getNotes();
  const result: Array<{ noteId: string; noteContent: string; todo: TodoItem; folderIds: string[] }> = [];
